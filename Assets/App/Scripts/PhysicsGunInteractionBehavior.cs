...
public class PhysicsGunInteractionBehavior : MonoBehaviour
{
    ...
    #region ObjectPush
            if (Input.GetMouseButtonDown(2))
            {
                ...
                _justReleased = true;
                _grabbedRigidbody.AddForce(PlayerTransform.forward * _throwpower, ForceMode.Impulse);
                ReleaseObject();
            }
        }
	}
    ...
    /// <returns>Ray from center of the main camera's viewport forward (used in ObjectHold and ObjectMove)</returns>
    private Ray CenterRay()
    {
        return _camera.ViewportPointToRay(_oneVector3 * 0.5f);
    }
    #region ObjectRelease
    private void ReleaseObject()
    {
        _grabbedRigidbody.gameObject.GetComponent<Outline>().Hide();
